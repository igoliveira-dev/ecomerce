...
import creditCard from '../../assets/images/cartao-de-credito.png'
...
const Checkout = () => {
  ...
  const form = useFormik({
    ...
    validationSchema: Yup.object({
      fullName: Yup.string().min(5, 'Nome completo inválido').required('O campo é obrigatório'),
      ...
      cardDisplayName: Yup.string().when((values, schema) => payWithCard ? schema.required('O campo é obrigatório') : schema),
      cardNumber: Yup.string().when((values, schema) => payWithCard ? schema.required('O campo é obrigatório') : schema),
      ...
      installments: Yup.number().when((values, schema) => payWithCard ? schema.required('O campo é obrigatório') : schema),
    }),
    onSubmit: (values) => {
      purchase({
        billing: {
  ...
  return (
    <div className='container'>
              ...
              <>
                ...
                <S.Row marginTop="24px">
                  <S.InputGroup maxWidth="150px">
                    ...
                    <select onChange={form.handleChange} onBlur={form.handleBlur} id='installments' name='installments' value={form.values.installments} className={checkInputHasError('installments') ? 'error' : ''}>
      ...
      <Button type="submit" title="Clique aqui para finalizar a compra" disabled={isLoading}>
